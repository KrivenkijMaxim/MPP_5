...
namespace TestProject
{
    public class Pipeline
    {
        public Task Generate(int maxDegreeOfParallelism, string destination, params string[] paths)
        {
            ...
            var newOptions = new DataflowLinkOptions { PropagateCompletion = true }; //формирование трубопровода
            openFile.LinkTo(generateTests, newOptions);
            generateTests.LinkTo(writeToFile, newOptions);


            foreach (string path in paths)
                openFile.Post(path); //отправка данных в конвейер
            openFile.Complete(); //завершение деятельности конвейера

            return writeToFile.Completion; //ожидание завершения трубопровода
        }
    }
}